...
- **Phase 8 multi-sheet Excel export complete — PR [#26](https://github.com/saqlainmmomin/Rail-PVC/pull/26) open on `codex-phase8-export-ui` (2026-07-23).** The approved-run Excel endpoint now emits `Cover`, `Bill`, and `W Derivation`: contract/LOA metadata plus approved bill history, the unchanged WS-B submission-parity Bill table, and persisted named W deductions with prior negative PVC carry-forward labeled separately. Legacy approved runs with nullable migration-015 totals fall back to summed component values. Verification: export tests **16/16**, current backend **232 passed / 2 skipped**, frontend typecheck + lint + production build clean; 101 non-schema Vitest tests pass while the unrelated in-progress contract-cleanup work leaves `lib/api/schema.test.ts` waiting on schema regeneration. LibreOffice smoke rendered exactly three readable pages with no clipped/split columns.
- **Invite-only tenant provisioning — PR [#23](https://github.com/saqlainmmomin/Rail-PVC/pull/23) merged to `main` (2026-07-20).** Migration **019** is applied and verified on the live Supabase DB; `tenant_invites` is forced-RLS with no client-visible rows, normalized unique emails, and FK-backed tenant ownership. Race-safe first-login invite consumption plus the idempotent tenant/invite operator script shipped with backend suite **194/194**. Production demo seeding and browser smoke remain blocked by the DEMO-2 reconciliation finding below and by the need for a controlled test email.
- **Parallel backlog cleared on `saqlain/parallel-backlog` (2026-07-17, Fable orchestrator session).** Four workstreams from `tasks/handoffs/2026-07-17-fable-parallel-backlog.md`, one commit each: **WS-A** usability-audit triage (new `AUDIT-1` section in TASKS.md; audit's 3 BLOCKERs verified already fixed; 2 quick wins shipped — gross-amount help note, contracts-list Value ₹ column; 2 tickets open for Saqlain); **WS-B** P8-REVIEW export parity first pass (submission Bill-sheet column order/headers, native numeric cells + number formats, live `=SUM()` total, quarter in summary; gap report in the handoff Results — multi-sheet audit trail, steel sub-lines, cover page remain open); **WS-C** migration **017** `contracts_base_month_first_day` CHECK (KU1R-L1 closed; applied to live DB, day≠1 INSERT verified rejected — **DB now at head 017**); **WS-D** ItemsGrid migrated to AG Grid v35 rowSelection object API (deprecated options gone repo-wide). Suites: backend **171** (167+4 export pins), engine untouched, frontend 65 vitest + tsc/eslint/build clean. ~~Caveat: authenticated browser click-through was blocked by session permissions~~ — **closed 2026-07-18**: Codex ran the authenticated pass (Value ₹ column incl. null `—`, help note on both bill forms, 0 AG Grid deprecation warnings); evidence in `tasks/handoffs/2026-07-18-codex-browser-smoke-caveat.md` Results. PR #19 fully verified.
...
- **AUDIT13-14 deployment gate:** code remediation is complete and the review is closed, but migration **020** has not been applied. Apply/verify 020 before deploying the backend; new DELETE code writes `document_cleanup_jobs` and is intentionally not compatible with DB head 019. Protected browser smoke also remains to be run with an authenticated test session.
- **DEMO-2 seed reconciliation blocks the first live demo seed.** FK ordering, decimal use, base month/zone, and sequential idempotency passed review, and the unsafe default-tenant fallback was removed. However, current service-shaped inputs plus current authoritative index rows calculate BCT-24-25-252 at Bill 1 `63,253.98` and Bill 2 `100,772.51`, not the pinned fixture outputs (`0.00` and `76,959.55`). The mismatch involves technical-withheld mapping, carry-forward derivation, and historical-vs-current index observations. The task forbids changing fixture numbers or engine code, so a domain/data decision is required before production seeding.
...
- **`AUDIT13-14-REVIEW` CLOSED (2026-07-23)** — all 2 HIGH, 4 MEDIUM, and 1 LOW findings remediated; follow-up adversarial/reliability findings also fixed. Draft deletion now preserves audit history, durably and autonomously cleans document objects, and returns without Storage latency; percent inputs retain the historical fractional domain with tested submit/display conversions; OpenAPI and cache behavior are pinned. Full suites: backend **243 passed / 4 isolated-Postgres skips**, frontend **102 passed**, tsc/eslint/build clean. Deployment requires migration 020 first.
...
- Suite state: **194/194 backend** on `codex/tenant-demo-provisioning`; on `main`, **180/180 backend**, **136 passed + 7 xfailed engine**, `mypy engine` + `tsc` + `eslint` clean.
...
- **`tenderaudit.in` is live in production (2026-07-19) — DEPLOY.md go-live checklist (steps 1–5) complete.** Backend on Railway (`api.tenderaudit.in`), frontend on Vercel (`tenderaudit.in`/`www`), GoDaddy DNS wired (stale parking records removed), Supabase auth redirect URLs updated, `/health` verified, real-account login confirmed. `frontend/middleware.ts` renamed to `proxy.ts` (Next 16 deprecated the old convention; legacy `middleware.ts` runs on Edge Runtime, which broke on `@supabase/ssr`'s Node-only deps). Full session: [tasks/session-log-2026-07-19.md](tasks/session-log-2026-07-19.md).
- **Landing page + auth redesign (2026-07-19 evening, on `main`).** `/` is now a public marketing landing page (`frontend/app/page.tsx`; `proxy.ts` allows it logged-out, logged-in still redirects to `/contracts`); auth pages restyled as a split-panel layout (`(auth)/layout.tsx`) with no logic changes. `next build` clean; browser-smoked landing/login/signup at desktop + mobile widths. Second design pass due before first outside exposure. Log: [tasks/session-log-2026-07-19.md](tasks/session-log-2026-07-19.md) (20:43 entry).
- **`main` includes tenant provisioning via merge `fca35c9`.** Migration 019 was applied to the live Supabase DB on 2026-07-19 (`alembic current` = 019); rollback-only constraint/RLS probes passed and `api.tenderaudit.in/health` is OK. Supabase is unpaused. Railway's deployment of the merge has not been version-verifiable from the health endpoint.
