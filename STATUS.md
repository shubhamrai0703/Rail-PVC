...
- **Invite-only tenant provisioning — `codex/tenant-demo-provisioning` (2026-07-19).** Migration **019** is applied and verified on the live Supabase DB; `tenant_invites` is forced-RLS with no client-visible rows, normalized unique emails, and FK-backed tenant ownership. The branch adds race-safe first-login invite consumption plus an idempotent tenant/invite operator script; backend suite **194/194**. Production demo seeding and browser smoke remain blocked by the DEMO-2 reconciliation finding below and by the need for a controlled test email.
...
- **`codex/tenant-demo-provisioning` branches from current `main`.** Migration 019 was applied to the live Supabase DB on 2026-07-19 (`alembic current` = 019); rollback-only constraint/RLS probes passed and `api.tenderaudit.in/health` is OK. Supabase is unpaused. The auth hook is not live until this branch merges and Railway deploys it.
